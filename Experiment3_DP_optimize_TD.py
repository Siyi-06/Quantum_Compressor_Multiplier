...
if __name__ == "__main__":
    ...
    for METRIC_INDEX in [1]:
        ...
        for N in range(3, max_N + 1):
            ...
            best_qc_2_y.append(2*N*N + N + 2*N)
        ...
        if METRICS[METRIC_INDEX] == 'T-Depth':
            plt.plot(x, best_td_y, label=f"Best Previous TD", color='grey')
            plt.plot(x, dp_td_y, label=f"TD DP", color='red')


        if METRICS[METRIC_INDEX] == 'Qubit-Count':
            # plt.plot(x, best_qc_y, label=f"Best Previous QC", color='red')
            plt.plot(x, best_qc_2_y, label=f"Best Previous QC", color='red')
