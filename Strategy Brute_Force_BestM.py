
import math
def min_index(lst_int):
    index = []
    min_n = min(lst_int)
    for i in range(len(lst_int)):
        if lst_int[i] == min_n:
            index.append(i)
    return index  #返回一个列表


# Parameters
N = 600  # Size of the multiplier (N x N)
#max_m = N  # Maximum value of m for m:k compressors
#print(P_value)
Cost=[]#
TD_List=[]
TC_List=[]
QC_List=[]
max_m_List=[]
add_size_List=[]
for max_m in range(2,N+1):
    max_layers = N  # Maximum number of layers in the Wallace tree, not include the last Carry Propagation Adder
    max_columns = 2 * N - 1  # Maximum columns in Wallace Tree
    P_value = [[0 for l in range(max_layers)] for j in range(max_columns)]
    #print(max_m)
    # First Layer: Initialize the partial product count
    for j in range(N):
        P_value[j][0] = j + 1
        # print("!", j, j + 1)
        if j!=N-1:
            P_value[max_columns - 1 - j][0] = j + 1
            # print("!",max_columns -1- j, j + 1)
    #print(P_value)

    #Cost of First Layer
    TC=4*N*N
    TD=N
    QC=2*N+N*N
    #print("Cost of First Layer:")
    #print("TC=",TC,"TD=",TD,"QC=",QC)

    # Compressor constraints and partial product updates
    flag_value=3
    add_size=100
    m_largest=100
    m=10000
    k=10000
    for l in range(max_layers):
        #flag for check last layer
        m_largest=0
        flag_value=0
        add_size=0
        #print("flag!!!", flag_value)
        for j in range(max_columns):
            if add_size==0:
                if P_value[j][l]==2:
                    add_size=max_columns-j
            if P_value[j][l] > 2:
                flag_value = 1
        if flag_value==0:
            print("flag!!!", flag_value)
            print("max_l=",l)#maximum level 3 layers, not include CPA
            print("add_size=", add_size)
            break
        for j in range(max_columns):
            # Apply compressors and update the partial products
            if P_value[j][l] > 1:
                m = P_value[j][l]  # use the largest compressor always
                k = math.floor(math.log2(m)) + 1
            if P_value[j][l]<2:
                m=0
                k=0
                #print("P_value[", j, ",", l, "]", P_value[j][l], "m", m, "k", k)
                #print("No compressor")
                continue
            if l == 0:
                if P_value[j][l] > max_m:
                    m = max_m
                    k = math.floor(math.log2(m)) + 1
                    #print("YES")
                #print("P_value[", j, ",", l, "]", P_value[j][l], "m", m, "k", k)
            if l>0:
                if P_value[j][l] > 2:
                    m=3
                    k=2
                if P_value[j][l]==2:
                    m=2
                    k=2
                if P_value[j][l] < 2:
                    #print("No Compressor Here!")
                    m=0
                    k=0
                    continue
                #print("P_value[",j,",",l,"]",P_value[j][l],"m",m,"k",k)
                ###Update Cost
            #print("m_largest", m_largest)
            if m_largest<m:
                m_largest=m
                #print("m_largest",m_largest)
            if m > k:
                TC = TC + 4 * (m - k)
                QC = QC + (m - k)
            if m == k:
                TC = TC + 4
                QC = QC + 1
                #print("!!!B2")

                # Update the partial product count for column j after applying the compressor
                # Reduce the partial products at column j
            if l < (max_layers-1):
                    # Reduce the partial products at column j
                P_value[j][l + 1] = P_value[j][l] - m + 1 + P_value[j][l + 1]
                    # print("Compression")
                    # print("P_value[", j, ",", l, "]", P_value[j][l])
                    # print("P_value[", j, ",", l+1, "]", P_value[j][l + 1])

                    # Propagate carry-out bits to the next columns (j+1 to j+k-1)
                for ii in range(1,k):
                    if j + ii < 2 * N - 1:
                            # print("Before carry propagation")
                            # print("P_value[", j + ii, ",", l + 1, "]", P_value[j + ii][l + 1])
                        P_value[j+ii][l+1] = P_value[j+ii][l+1]+1
                        #print("carry propagation")
                            # print("P_value[", j+ii, ",", l+1 , "]", P_value[j+ii][l+1])
        #Update Cost
        #print("Previous TD", TD)
        TD=TD+math.floor(math.log(m_largest,3))+2*math.floor(math.log(m_largest,2))-1
        #print("m_largest", m_largest)
        #print("TD",TD)
    print("max_m",max_m)
    print("Cost", "TC=",TC,"TD=",TD,"QC=",QC)
    Cost.append([max_m,TC,TD,QC,add_size])
    max_m_List.append(max_m)
    TC_List.append(TC)
    TD_List.append(TD)
    QC_List.append(QC)
    add_size_List.append(add_size)
print(Cost)

#TC
print(min_index(TC_List))
print(Cost[TC_List.index(min(TC_List))])
#TD
print(min_index(TD_List))
print(Cost[TD_List.index(min(TD_List))])
#QC
print(min_index(QC_List))
print(Cost[QC_List.index(min(QC_List))])

if min_index(TD_List).count(min_index(TC_List)[0])>0:
    if min_index(TD_List).count(min_index(QC_List)[0])>0:
        print("Yes, Best TC TD QC")
        print("Best M",max_m_List[min_index(TC_List)[0]])
